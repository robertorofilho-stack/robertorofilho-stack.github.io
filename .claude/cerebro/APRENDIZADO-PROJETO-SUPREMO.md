...
### 29. Memória tem data; saldo não — pedido de dinheiro só sai com prova viva
Mandei o operador recarregar o Gemini porque a memória de 06/09 dizia "crédito zerado". A `description:` do mesmo arquivo dizia "RESOLVIDO 07/09 — recarregou, HTTP 200". Li o nome no recall e o corpo pela metade. É o #28 com outra cara: lá pedi criar o que existia; aqui pedi pagar o que já estava pago. A raiz é a mesma — tratar um registro como estado. Regra: pedido de dinheiro (recarregar, assinar, comprar) só sai com prova viva — HTTP 429 de uma sonda real, ou uma memória mais nova que confirme a falta; e quando o arquivo tem `description:`, ela é o estado atual e o corpo é histórico.
**Corrigido em código (mesma sessão):** `conselho.py --saldo` sonda a chave Gemini de verdade (200/429/404, cada um com significado e sem chave na URL); `recall.sh` injeta a `description:` de cada memória do mestre e um bloco 💳 de estado vivo quando o pedido cheira a pagamento. De quebra achei que o regex `cr[ée]dito` do bloco 🔑 nunca casava "crédito" em locale C — a nuvem só oferece a grafia `C.utf8` e o hook procurava `C.UTF-8` exato, então rodava em POSIX e o recall de credenciais estava mudo para essa palavra desde o #28. Regex portável (`cr.{1,2}dito`) + loop de locale aceitando as duas grafias, testado em C e C.UTF-8 com o pedido real: "abre um link para eu colocar crédito" → o recall mostra "gemini-sem-credito.md — RESOLVIDO 07/09/2026…" antes de eu responder. Verificador que depende de locale é verificador calado (#27 de novo).

### 30. Clone de ontem é cegueira de silo — `git pull` no mestre ANTES do recall
Recebi a ordem de validar e construir o app de cirurgias, rodei o protocolo inteiro (recall, Plaud, arquiteto, auditor, qa, conselho) e construí um MVP verificado com 40 checagens. Só que o Mac mini já tinha construído e publicado o app em 10–11/09, com dados reais e o operador logado. O recall leu um clone do mestre de 10/09: o `add_repo` do começo da sessão tirou uma foto, e eu nunca a atualizei — o "fetch antes de push" que a própria ferramenta recomendou só aconteceu na hora de publicar, tarde demais. Um dia inteiro refazendo o que existia, a versão mais cara do #25 (cegueira de silo) e do #2 (inventário antes de construir). Regra: na nuvem, o clone do mestre é uma foto; puxar antes de ler, sempre, e desconfiar de qualquer "não existe" vindo de um clone que não foi atualizado hoje.
**Corrigido em código (mesma sessão):** `.claude/hooks/_mestre.sh` — `atualizar_mestre_nuvem` faz `git pull --ff-only` no clone (só Linux, timeout 25 s, no máximo a cada 30 min no recall, sempre no início da sessão) e injeta no contexto "MESTRE ATUALIZADO (a → b): N commits novos" ou "MESTRE NÃO ATUALIZADO: clone pode estar velho". Chamado por `carregar-cerebro.sh` e `recall.sh`. Testado ao vivo: o pull rodou, o carimbo foi gravado e a segunda chamada em 30 min foi pulada. CLAUDE.md §0 passo 1 ganhou a frase. O trabalho paralelo não foi apagado (Lei da Monotonia): virou pacote de referência no dossiê do mestre, com README dizendo o que aproveitar (auditoria de 20 fontes, conselho 4/4, docs LGPD, módulos candidatos).
